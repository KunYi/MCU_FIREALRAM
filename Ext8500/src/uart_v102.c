  ...
  if (TI) {                                // Is TI flag set
    ...
    if (TLength0 != 0 ) {                  // UART0 Tx FIFO is empty
      ...
      if (TLength0 == 0) {
        ...
        SCON0 = 0xF0;                       // switch to 9bits mode
      }
      ...
      SCON0 = 0x50;                        // switch to 8bits mode
...
void UART1_Interrupt(void) interrupt 7  using 2 {
  ...
  if (TI1) {                                // Is TI flag set
    ...
    if (TLength1 != 0 ) {                   // UART1 Tx FIFO is empty
      TLength1--;
      // last byte send will be change transmit mode to 9bit
      if (TLength1 == 0) {
        TB81  = 1;                          // dummy stop bit
        SCON1 = 0xF0;                       // switch to 9bits mode
      }    
      ...
      SCON1 = 0x50;                         // switch to 8bits mode
